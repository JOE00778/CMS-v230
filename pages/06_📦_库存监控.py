...
旧 page33 库存构成分析 + 旧 page06 库存健康监控 を 1 ページ 2 タブに統合。
- tab1 构成分析：JD+弁天 用途/等级構成（内部に 棚別/SKU別/NST-JDL対账/JDL照会 の 4 子タブ）
- tab2 健康监控：健康度 4 档 + 等级×健康 矩阵 + 死钱清单
各 body は関数化し st.stop()→return（外側タブで両方が毎回実行されるため）。
"""
...
tab_comp, tab_health = st.tabs([t("📦 库存构成分析"), t("📦 库存健康监控")])
with tab_comp:
    _render_composition()
with tab_health:
    _render_health()
