"""模块 #31 入荷实绩 · 受領書(ItemReceipt)ベースの実際入荷.
...
3 tab（Boss 2026-06-25 拍板）:
  tab1 入荷历史    · 哪张收货单何时、从哪个供货商、收了什么、多少钱
  tab2 入荷实绩vs预定 · 有效预定日(手動ETA/page30优先) 与实际收货日的差 + 发注→收货提前期
  tab3 检收金额    · item别 / 月别 检收金额（收货原价）
...
st.set_page_config(page_title=t("入荷实绩"), page_icon="📥", layout="wide")
...
def _vendor_options() -> list[tuple[str, str]]:
    """(vendor_id, vendor_name) 一覧（有收货实绩的供货商のみ）。"""
