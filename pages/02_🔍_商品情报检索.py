...
with _tab_compare:
    ...
    """)
    if cmp_err:
        st.error(_L("数据读取失败：", "データ読み取り失敗：") + cmp_err)
    elif cmp_df is None or cmp_df.empty:
        ...
        # 计算 diff（NST package vs JDL wms）
        cmp_df["diff_g"] = cmp_df["jdl_wms_g"] - cmp_df["nst_package_g"]
        cmp_df["diff_pct"] = (
            cmp_df["diff_g"] / cmp_df["nst_package_g"].where(cmp_df["nst_package_g"] > 0)
        ) * 100

        # 覆盖率统计
        n_total = len(cmp_df)
        n_nst = int(cmp_df["nst_package_g"].gt(0).sum())
        n_jdl = int(cmp_df["jdl_wms_g"].gt(0).sum())
        comparable = cmp_df[
            cmp_df["nst_package_g"].gt(0) & cmp_df["jdl_wms_g"].gt(0)
        ].copy()
        n_cmp = len(comparable)
        n_close = int(comparable["diff_pct"].abs().le(10).sum())    # 差 ≤ 10%
        n_diff_big = int(comparable["diff_pct"].abs().gt(30).sum()) # 差 > 30%

        k1, k2, k3, k4, k5 = st.columns(5)
        k1.metric(_L("活跃 SKU", "アクティブ SKU"), f"{n_total:,}")
        k2.metric(_L("NST 有重量", "NST 重量あり"), f"{n_nst:,}",
                  f"{n_nst/n_total*100:.0f}%" if n_total else "")
        k3.metric(_L("JDL 有重量", "JDL 重量あり"), f"{n_jdl:,}",
                  f"{n_jdl/n_total*100:.0f}%" if n_total else "")
        k4.metric(_L("可对比", "比較可能"), f"{n_cmp:,}",
                  f"{n_cmp/n_total*100:.0f}%" if n_total else "")
        k5.metric(
            _L("差 ≤ 10%", "差 ≤ 10%"), f"{n_close:,}",
            f"{n_close/n_cmp*100:.0f}%" if n_cmp else "",
            help=_L("一致性较好的占比", "一致性良好の割合"),
        )

        if n_cmp > 0:
            st.divider()
            # 差异大的 SKU 优先（按 |diff_pct| 降序）
            comparable_sorted = comparable.copy()
            comparable_sorted["abs_diff_pct"] = comparable_sorted["diff_pct"].abs()
            comparable_sorted = comparable_sorted.sort_values(
            ...
            csv2 = comparable_sorted[show_cols].to_csv(index=False).encode("utf-8-sig")
            st.download_button(
