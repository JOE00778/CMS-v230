"""模块 #34 供货商管理 v2 · 采购可视化看板（第一期 · spec 2026-07-15 a5138cf）.
  ...
  📊 供货商数据大盘 供应商×月：需求基线/计划基线金额/实际PO/差异/起订·免运缺口/理论节省
...
st.title(t("🏢 供货商管理"))
st.caption(_dl(
    "采购可视化看板（第一期）：供货商数据大盘 · 品牌×供应商 · 折扣率总览 · SKU采购明细 · "
    "优化机会 · 报价维护 · 供应商与品牌规则。最低报价仅为理论优化机会，不产生订货指令。",
    "仕入可視化ダッシュボード（第1期）：仕入先ダッシュボード · ブランド×仕入先 · 掛率総覧 · "
...
with tab_sku:
    ...
    if _fs.empty:
        ...
        #      moq          = 見積 DB に入力があればそれ、無ければ **NST 箱規(carton_qty) で代替**
        #                     （隋艶偉 2026-09-15「起订量的位置用 NST 的箱规数据替代」）
        ...
        _bulk_q = _read(
            ...
            "q.price, coalesce(q.moq, im.carton_qty) AS moq, q.order_lot, "
            "im.order_lot AS nst_order_lot, im.carton_qty, "
        ...
        if not _bulk_q.empty:
            _bulk_show = _bulk_q.rename(columns={
                ...
                "moq": _dl("起订量(报价·空则NST箱规)", "最低発注数(見積·無ければNST箱規)"),
