...
結果は 3 種:
  정상  名義と통관고유부호が一致
  오류  存在しない / 名前不一致 / 13桁でない / P 始まりでない
...
def apply_results(rows: list[dict], smap: dict[str, dict], field: str = "AA") -> int:
    """検証結果を ECMS 行に反映する。戻り値は落とした件数。

    · ng（存在しない / 名前不一致 / 形式不正）→ **PCCC を落とす**
      （許慧杰さん運用「通关号码不对的话上传ecms系统之前会摘掉」と同じ）
    · fault（関税庁側の障害・通信障害）→ **落とさない**。相手が一時的に落ちているだけで
      番号自体は正しいかもしれない。ここで消すと正しい PCCC を捨てることになる
    · ok / 結果なし → そのまま
    """
    dropped = 0
    for r in rows:
        hit = smap.get((r.get(field) or "").strip().upper())
        if hit and hit["status"] == STATUS_NG:
            r[field] = ""
            dropped += 1
    return dropped
