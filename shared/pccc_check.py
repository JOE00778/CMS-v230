...
照合される項目（2026-09-08 に運営の実データ 44 件で**実測**。推測ではない）:
   氏名     照合する。違えば 오류「납세의무자명이 ... 일치하지 않습니다」
   電話     照合する。違えば 오류「납세의무자 전화번호가 일치하지 않습니다」
   郵便番号 **登録済みの PCCC だけ**照合する。44 件中 6 件（14%）がそれで、
            残り 38 件は郵便番号を 99999 にしても 정상 のまま。空にすると 장애。
            しかも**文字列比較**——`47515` は 정상 だが `047515` は 오류。

⚠️ 郵便番号は**5 桁のまま**渡すこと（`coupang_to_ecms_xlsx.zip5()` を通す）。
   Excel 由来の値は前ゼロが落ちて `02585` → `2585` になっており、そのまま送ると
   ソウルの客（01xxx〜09xxx）が**全員** 오류 になる。2026-09-08 の運営報告
   「PCCC显示错误这部分应该是邮编格式的问题」はまさにこれで、指摘どおりだった。

⚠️ **오류 ≠ PCCC が無効**（2026-09-08 運営指摘「我人工核后确认实际信息是正确的，
   但是这个功能显示是错误的」）。上の郵便番号バグを直してもなお、引っ越し・番号変更の
   あと関税庁側を更新していない、職場や家族の住所に送っている——それだけで 오류 に
   なるが、番号自体は生きていて通関もできる（特送業者が受荷主情報を確認・修正して
   申告する運用）。出典: 한국관세무역개발원「등록된 우편번호까지 일치해야 통관 가능」
