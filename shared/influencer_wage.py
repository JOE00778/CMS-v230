...
# Shopee のエクスポート表頭は何度も変わっている（'时长：'に全角コロン / '销售(已确认订单数)'に
# 「金额」なし / '开始时间'に「直播」なし / '数据时段' vs '数据期间'）。記号·空白を落とした
# キーで下流が使う標準列名へ寄せる。新しい表記を見つけたらここに 1 行足すだけ。
COLUMN_ALIASES = {
    "直播名称": "直播名称",
    "时长": "时长",
    "销售金额已确认订单": "销售金额(已确认订单)",
    "销售已确认订单数": "销售金额(已确认订单)",
    "销售已确认订单": "销售金额(已确认订单)",
    "订单数已确认订单": "订单数(已确认订单)",
    "订单已确认订单数": "订单数(已确认订单)",
...
def canon_column(name) -> str:
    """列名 → 標準列名。別名表に無ければ strip しただけの原名を返す。"""
    key = re.sub(r"[\s:：()（）]", "", str(name))
    return COLUMN_ALIASES.get(key, str(name).strip())


def parse_period_month(s) -> str:
    """'01-08-2026 - 31-08-2026' / '8/1-8/31' → '8月'。取れなければ ''。"""
    text = str(s or "").strip()
    m = re.search(r"\d{1,2}-(\d{1,2})-\d{4}", text)  # DD-MM-YYYY
    if m:
        return f"{int(m.group(1))}月"
    m = re.match(r"\s*(\d{1,2})\s*/", text)          # M/D...
