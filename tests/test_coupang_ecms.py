...
@pytest.mark.parametrize("addr,expect", [
    # 「地址分析 ecms」シートの実データがそのまま正解
    ("경기도 수원시 권선구 호매실동 1388 호매실경남아너스빌 1511동 1401호",
     ("경기도", "경기도 수원시", "권선구 호매실동 1388 호매실경남아너스빌 1511동 1401호")),
    ("부산광역시 금정구 동부곡로6번길 22 동국쉐르빌 502호 ( 부곡동 )",
     ("부산광역시", "부산광역시", "금정구 동부곡로6번길 22 동국쉐르빌 502호 ( 부곡동 )")),
    ("서울특별시 강동구 둔촌동 555 둔촌신동아아파트 103동 1001호",
     ("서울특별시", "서울특별시", "강동구 둔촌동 555 둔촌신동아아파트 103동 1001호")),
    # 도 + 군
    ("전라남도 완도군 완도읍 개포로 12",
     ("전라남도", "전라남도 완도군", "완도읍 개포로 12")),
    # 略記は正式名へ寄せる
    ("서울 강남구 테헤란로 152", ("서울특별시", "서울특별시", "강남구 테헤란로 152")),
    ("경기 성남시 분당구 판교역로 235", ("경기도", "경기도 성남시", "분당구 판교역로 235")),
    # 旧称も受ける（강원도 → 강원특별자치도 / 전라북도 → 전북특별자치도 の改称前データ）
    ("강원도 춘천시 중앙로 1", ("강원도", "강원도 춘천시", "중앙로 1")),
    ("세종특별자치시 한누리대로 2130", ("세종특별자치시", "세종특별자치시", "한누리대로 2130")),
])
def test_住所分割(addr, expect):
    assert ce.split_address(addr) == expect


def test_未知の住所は推測で埋めない():
    """先頭が既知の시도でなければ空を返す。勝手に埋めると通関で弾かれる。"""
    assert ce.split_address("Tokyo Shibuya 1-2-3") == ("", "", "Tokyo Shibuya 1-2-3")
    assert ce.split_address("") == ("", "", "")
    assert ce.split_address("   ") == ("", "", "")


def test_余分な空白は潰す():
    assert ce.split_address("서울특별시   강남구  테헤란로 152")[2] == "강남구 테헤란로 152"
